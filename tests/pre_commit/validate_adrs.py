...
def main() -> None:
    ...
    # -------------------------------------------------------------------------
    adr_filename_pattern = re.compile(r"ADR-(\d{3,4})-.*\.md$", re.IGNORECASE)
    adr_title_pattern = re.compile(r"#\s*ADR[-\s]?0*(\d+)\s*:?", re.IGNORECASE)
    status_extract_pattern = re.compile(r"\*\*Status:\*\*\s*(.*)")
    superseded_status_pattern = re.compile(
        r"^Superseded by ADR-\d{3,4}$", re.IGNORECASE
    )
    version_date_extract_pattern = re.compile(r"\*\*Version/Date:\*\*\s*(.*)")
    version_date_validation_pattern = re.compile(r"^v\d+\.\d+\s*/\s*\d{4}-\d{2}-\d{2}$")

    # -------------------------------------------------------------------------
    # 4. Iterate & Validate each Active ADR document
    # -------------------------------------------------------------------------
    for file_path in sorted(ROOT_DIR.iterdir()):
        ...
        m = adr_filename_pattern.match(filename)
        if not m:
        ...
        try:
            ...
            m2 = adr_title_pattern.match(first_line)
            if not m2:
            ...
            status_match = status_extract_pattern.search(content)
            if status_match:
                status = status_match.group(1).strip()
                allowed_statuses = {"Proposed", "Accepted", "Rejected", "Deprecated"}
                is_valid_status = (
                    status in allowed_statuses
                    or superseded_status_pattern.match(status)
                )
            ...
            version_match = version_date_extract_pattern.search(content)
            if version_match:
                version_str = version_match.group(1).strip()
                if not version_date_validation_pattern.match(version_str):
                    issues.append(
