...
def save_json_with_observability(
    ...
    if isinstance(payload, dict):
        ...
        if sort_keys:
            # Sort non-metadata keys alphabetically
            non_meta_keys = sorted(
                k for k in payload if k not in ("_warning", "_generated_at")
            )
            ordered_payload = metadata
            for k in non_meta_keys:
                ordered_payload[k] = payload[k]
            payload = ordered_payload
            sort_keys = False  # Prevent json.dump from overriding our custom order
        else:
            ordered_payload = metadata
